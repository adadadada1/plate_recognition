...
class DExplorer:
        ...
        with torch.no_grad():
            ...
            for point, c in points:
                ...
                for x, y in [[x1, y1], [x2, y2], [x3, y3], [x4, y4]]:
                    ...
                    i += 1
                conf_text = f"Confidence: {c:.5f}"
                image = cv2.putText(image, conf_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
                                    1, (0, 255, 0), 2)
    ...
    def select_box(self, predict, size, dims=208, stride=16):
        ...
        affines = torch.cat(
            (
                predict[0, :, :, 2:3], predict[0, :, :, 3:4], predict[0, :, :, 4:5],
                predict[0, :, :, 5:6], predict[0, :, :, 6:7], predict[0, :, :, 7:8]
            ),
            dim=2
        )
        h, w, c = affines.shape
...
if __name__ == '__main__':
    ...
    e = DExplorer()
    image = cv2.imread('test/test1.bmp')
